...
parser.add_argument('--batch_size', type=int, default=8, help='batch size (per 1GPU)')
...
parser.add_argument('--save', type=str, default='DFDC', help='experiment name')
...
def main(train_queue, valid_queue, pos_weight=1):
  ...
  torch.cuda.empty_cache()
  valid_acc, valid_obj = infer(valid_queue, model, criterion)
  logging.info('valid_acc (last test) %f', valid_acc)
  torch.cuda.empty_cache()
...
def train(train_queue, valid_queue, model, architect, criterion, optimizer, lr, epoch, scheduler, start_step=0):
  ...
  for step, (input, target) in enumerate(train_queue,start=1):
    ...
    target_search = target_search.to(device)#.requires_grad_(False)
...
if __name__ == '__main__':
  ...
  data_num, class_file_num, class_weights = getDfdcDataLoader(
  ...
  main(train_dataloader, validation_dataloader, class_weights[1])
