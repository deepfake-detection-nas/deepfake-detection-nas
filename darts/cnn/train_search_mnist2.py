...
parser.add_argument('--batch_size', type=int, default=128, help='batch size (per 1GPU)')
...
parser.add_argument('--save', type=str, default='MNIST2', help='experiment name')
...
fh = logging.FileHandler(os.path.join(args.save, 'log.txt'), 'a')
...
def main(train_queue, valid_queue, pos_weight=1):
  ...
  # torchsummary.summary(model, image_size, device='cuda')
...
def train(train_queue, valid_queue, model, architect, criterion, optimizer, lr, epoch, scheduler, start_step=0):
  ...
  for step, (input, target) in enumerate(train_queue,start=1):
    ...
    input = input.to(device)#.requires_grad_(False)
    target = target.unsqueeze(1).float().to(device)#.requires_grad_(False)

    # get a random minibatch from the search queue with replacement
    input_search, target_search = next(iter(valid_queue))
    input_search = input_search.to(device)#.requires_grad_(False)
    target_search = target_search.unsqueeze(1).float().to(device)#.requires_grad_(False)
...
if __name__ == '__main__':
  ...
  train_indices = get_indices(train_data, [0, 1])
  ...
  test_indices = get_indices(test_data, [0, 1])
  test_data_subset = Subset(test_data, test_indices)
  test_loader = DataLoader(test_data_subset, batch_size=args.batch_size, shuffle=True)
