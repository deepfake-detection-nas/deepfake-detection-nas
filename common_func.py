...
### リストのランダムサンプリング ###
def sample_list(input_list, rate, seed=2):
    total_elements = len(input_list)
    num_to_sample = int(total_elements * rate)

    random.seed(seed)
    selected_indices = sorted(random.sample(range(total_elements), num_to_sample))
...
def makeImagePathList_Celeb(
    ...
    # ここで割合を削る（全体として削るならここ）
    train_data = sample_list(train_data,use_data_rate,seed)
    validation_data = sample_list(validation_data,use_data_rate,seed)
    test_data = sample_list(test_data,use_data_rate,seed)    
...
def getCelebDataLoader(
        ...
        validation_rate=0,
        test_rate=0,
        use_data_rate=1.0, #追加分
        seed=2, #追加分
        shuffle=False,
        num_workers=torch.cuda.device_count() # 通常モデルとpc-dartsでは0を指定
    ):
    ...
    if validation_rate > 0:
        ...
        validation_dataloader = DataLoader(validation_dataset, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers, pin_memory=True)
    else:
